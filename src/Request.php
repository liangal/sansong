...
//            'accessToken' => $this->getAccessToken(),
    ...
    {
        return $this->sendRequest($this->getUrlApi($url), $params, 'GET', $options);
    }
    ...
    {
        return $this->sendRequest($this->getUrlApi($url), $params, 'POST', $options);
    }
    ...
    public function sendRequest(string $url, array $data = [], string $method = 'POST', $options = []): array
    {
        ...
        if ('GET' == $method) {
            ...
            $defaults[CURLOPT_POSTFIELDS] = is_array($params) ? http_build_query($params) : $params;
        } else {
            $defaults[CURLOPT_URL] = $url;
    ...
    public function sendAsyncRequest($url, $params = [], $method = 'POST')
    {
        ...
        //发送socket请求,获得连接句柄
        $fp = fsockopen($parts['host'], !empty($parts['port']) ?: 80, $errno, $errstr, 3);
        if (!$fp) {
            return false;
        }
        //设置超时时间
